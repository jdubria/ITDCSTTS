<?php 
	include "../connection/conn.php";
				$rownums = array();
			 	...
			 	print_r($dayvalue);
			 	echo "<br>";
			 	$sql1 = faculty_conn1($facid, $time3, $sem_id, $sy_id, $size3);
			 	foreach ($dayvalue as $d) {
			 		$sql2 = faculty_conn2($facid, $d, $sem_id, $sy_id);
			 		$rownums[] = mysqli_num_rows($sql2);
			 	}
			 	$sql3 = faculty_conn1($facid, $time2, $sem_id, $sy_id, $size2);
				...
				if ($rownum1 > 2) {
					...
					if ($CD > 0 && $CS > 0) {
						...
						$newdays = array_slice($dayvalue, 1, $size3);
						echo "newdays"; print_r($newdays);
						echo "<br>arr";
						print_r($arr);
					...
					if ($isip > 0) {
					 	...
					 	$next_end = add2timevalue($MAXEND[$i], $time);
							...
							INSERT INTO `room_day_time` (`room_id`, `sem_id`, `sy_id`, `day_id`, `start_time`, `end_time`) VALUES ({$roomid}, {$sem_id}, {$sy_id}, {$dayvalue[$i]}, '{$MAXEND[$i]}', '{$next_end}'); 			
			 					");
						 	$res = mysqli_query($conn, "SELECT MAX(rdt_id) FROM room_day_time WHERE room_id = {$roomid}");
						 	$fetchs = mysqli_fetch_assoc($res);
					...
					INSERT INTO `itdcstts`.`room_day_time` (`room_id`, `sem_id`, `sy_id`, `day_id`, `start_time`, `end_time`) 
		 				...
		 				");
		 					$res = mysqli_query($conn, "SELECT MAX(rdt_id) FROM room_day_time");
						 	$fetchs = mysqli_fetch_assoc($res);
		 					$rdt_ids[] = $fetchs['MAX(rdt_id)']; 				
					 	}			 							
					 	foreach ($rdt_ids as $id) {
			      			  mysqli_query($conn, "
			         			INSERT INTO `itdcstts`.`room_sched` (`schedid`, `rdt_id`, `sem_id`, `sy_id`) VALUES ({$schedID}, {$id}, {$sem_id}, {$sy_id});		
			         			");
		         			}
				 			$labtrue = TRUE;
				 			echo "lab <= 3 - 7.2.4/2";						
